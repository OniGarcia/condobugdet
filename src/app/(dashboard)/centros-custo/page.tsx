...
export default async function CentrosCustoPage() {
  const [centros, allFlat, categoriaTree] = await Promise.all([
    getCentrosCusto(),
    getCategoriasFlat(),
    getCategoriasTree(),
  ])
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Centros de Custo</h1>
        <p className="text-neutral-400">
          Agrupe categorias do Plano de Contas para análise financeira especializada (ex: Taxa Condominial, Manutenção).
        </p>
      </div>

      <CostCenterView data={centros} allFlat={allFlat} categoriaTree={categoriaTree} />
    </div>
  )
}
