...
export default async function RealizadoPage({ searchParams }: { searchParams: Promise<{ ano?: string }> }) {
  ...
  const [categorias, realizados] = await Promise.all([
    getCategoriasTree(),
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-4rem)]">
      <div className="mb-6 flex justify-between items-end shrink-0">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Fluxo de Caixa Realizado</h1>
          <p className="text-neutral-400">Importe o Balancete Contábil ou preencha manualmente os lançamentos executados.</p>
        </div>
        
        <div className="flex gap-3 items-center bg-white/5 p-1 rounded-xl border border-white/10">
          <YearSelector years={availableYears} selectedYear={selectedYear} />
        </div>
      </div>

      {/* Main Realizado Data Grid Area */}
      <RealizadoGrid categorias={categorias} realizados={realizados} ano={selectedYear} />
    </div>
  )
}
...
function YearSelector({ years, selectedYear }: { years: number[], selectedYear: number }) {
  ...
  return (
    <div className="flex bg-neutral-900 rounded-lg overflow-hidden border border-white/5">
      {years.map(y => (
        <Link 
          key={y} 
          ...
          className={`px-4 py-2 text-sm font-medium transition-colors ${
            y === selectedYear 
              ...
              : 'text-neutral-400 hover:bg-white/10 hover:text-white block'
          }`}
        >
          {y}
        </Link>
